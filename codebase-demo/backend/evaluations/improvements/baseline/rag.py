...
def retrieve(repository, query_vector, count=5):
...
def build_context(chunks, question, max_output_tokens):
    ...
    budget = CONTEXT_TOKENS - max_output_tokens - len(SYSTEM_PROMPT.encode()) - len(question.encode()) - 512
    if budget < 256:
    ...
    for chunk in chunks:
        header = f"[{chunk['id']}] {chunk['path']}:{chunk['start_line']}-{chunk['end_line']}\n"
        remaining = min(budget, excerpt_budget) - len(header.encode()) - 2
        lines = []
...
def answer_question(repository, question, max_output_tokens):
    ...
    excerpts = build_context(retrieve(repository, query_vector), question, max_output_tokens)
    if not excerpts:
