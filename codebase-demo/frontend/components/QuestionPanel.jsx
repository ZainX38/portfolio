...
const EXAMPLE_QUESTION = 'When does raise_for_status raise an HTTPError?';
...
export default function QuestionPanel({ repository, onFileSelect, isDimmed }) {
    ...
    const [questionsRemaining, setQuestionsRemaining] = useState(repository.questions_remaining ?? 0);
    const canAsk = repository.ai_available && questionsRemaining > 0;
    ...
    function handleQuestionKeyDown(event) {
        if (event.key === 'Tab' && !question.trim() && canAsk && !isLoading) {
    ...
    async function submitQuestion(event) {
        ...
        setAnswer(null);
        try {
            const result = await askQuestion(repository.snapshot_id, question.trim());
            setAnswer(result);
            setQuestionsRemaining(result.questions_remaining);
        } catch (error) {
            setError(error.message);
            if (Number.isInteger(error.questionsRemaining)) {
                setQuestionsRemaining(error.questionsRemaining);
            }
        } finally {
            setIsLoading(false);
        }
        ...
        <section className="pointer-events-none absolute inset-x-0 bottom-0 z-20 px-3 pb-3 sm:px-5 sm:pb-5" aria-labelledby="question-heading">
            <div className={`pointer-events-auto mx-auto max-w-4xl transition-opacity duration-300 ${isDimmed ? 'opacity-55 hover:opacity-100 focus-within:opacity-100' : 'opacity-100'}`}>
                ...
                <form onSubmit={submitQuestion} className="rounded-3xl border border-sky-300/35 bg-slate-800/90 p-3 shadow-2xl shadow-slate-950/40 ring-1 ring-white/10 backdrop-blur-xl transition-colors duration-300 focus-within:border-sky-300 focus-within:bg-slate-800/95 focus-within:ring-2 focus-within:ring-sky-300/20 sm:p-4">
                    ...
                    {!canAsk && <p id="question-availability" className="px-2 pb-2 text-sm text-amber-200">{questionsRemaining === 0 && repository.ai_available ? 'You have used all three AI questions available today.' : repository.ai_message}</p>}
                    {canAsk && !question && (
                        <div className="flex justify-end px-2 pb-1">
                            <span id="question-completion-hint" className="rounded-md border border-sky-300/25 bg-slate-950/80 px-2 py-1 text-[11px] font-medium text-sky-200 shadow-lg">
                                Tab to complete
                            </span>
                        </div>
                    )}
                    <div className="flex items-end gap-3">
                        <textarea
                            ...
                            aria-describedby={!canAsk ? 'question-availability' : !question ? 'question-completion-hint' : undefined}
                        ...
                        <button
                            type="submit"
                            disabled={!canAsk || isLoading || !question.trim()}
                ...
                <p className="mt-2 text-center text-[11px] text-slate-500">Answers are grounded in the indexed repository. {questionsRemaining} AI question{questionsRemaining === 1 ? '' : 's'} remaining.</p>
            </div>
        </section>
    );
}
